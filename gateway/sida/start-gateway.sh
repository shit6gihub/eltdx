...
# 启动网关
echo "Starting eltdx-TQ Gateway on port ${GATEWAY_PORT:-17709}..."
nohup python3 eltdx-tq-gateway.py >> "$LOG_FILE" 2>&1 &
echo $! > "$PID_FILE"

sleep 3
if kill -0 $(cat "$PID_FILE") 2>/dev/null; then
    echo "Gateway started successfully (PID: $(cat $PID_FILE))"
    echo "Health check: $(curl -s http://127.0.0.1:${GATEWAY_PORT:-17709}/health)"
else
